...
impl TransactionalKvDatabase {
            ...
            let result = match operation.op_type.as_str() {
                ...
                "SET_VERSIONSTAMPED_KEY" => {
                    if let Some(value) = &operation.value {
                        ...
                        match utils::apply_versionstamp(&operation.key, commit_version, batch_order) {
                ...
                "SET_VERSIONSTAMPED_VALUE" => {
                    if let Some(value_prefix) = &operation.value {
                        ...
                        match utils::apply_versionstamp(value_prefix, commit_version, batch_order) {
    ...
    pub fn get(&self, key: &[u8]) -> Result<GetResult, String> {
        operations::Operations::get(&self.db, &self.current_version, key)
    }

    pub fn put(&self, key: &[u8], value: &[u8]) -> OpResult {
        operations::Operations::put(&self.db, &self.current_version, key, value)
    }

    pub fn delete(&self, key: &[u8]) -> OpResult {
        operations::Operations::delete(&self.db, &self.current_version, key)
    }

    pub fn list_keys(&self, prefix: &[u8], limit: u32) -> Result<Vec<Vec<u8>>, String> {
        operations::Operations::list_keys(&self.db, &self.current_version, prefix, limit)
    }
    ...
    pub fn calculate_offset_key(&self, base_key: &[u8], offset: i32) -> Vec<u8> {
        utils::calculate_offset_key(base_key, offset)
    }
...
mod tests {
    ...
    fn test_increment_key() {
        ...
        let result = utils::increment_key(&large_key);
    ...
    fn test_decrement_key() {
        ...
        let result = utils::decrement_key(b"\x00\x00\x00");
        ...
        let result = utils::decrement_key(b"test\x00\x00");
