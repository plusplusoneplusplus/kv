...
pub enum DatabaseFactory {
    ...
    Standalone { config: crate::lib::config::Config },
    /// Replicated mode - consensus-backed database (Phase 1)
    Replicated { config: crate::lib::config::Config, instance_id: u32 },
}
...
impl DatabaseFactory {
    ...
    pub fn standalone(config: crate::lib::config::Config) -> Self {
        DatabaseFactory::Standalone { config }
    }

    /// Create replicated factory
    pub fn replicated(config: crate::lib::config::Config, instance_id: u32) -> Self {
        DatabaseFactory::Replicated { config, instance_id }
    }
}
...
mod tests {
    use super::*;
    use crate::lib::config::{Config, DeploymentMode};
