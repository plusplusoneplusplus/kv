...
async fn test_versionstamped_key_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tx.set_versionstamped_key(
        b"user_score_\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
        b"100",
        None,
    )?;
    ...
    assert_eq!(
        commit_result.generated_keys.len(),
        1,
        "Should have one generated key"
    );
    assert_eq!(
        commit_result.generated_values.len(),
        0,
        "Should have no generated values for key operation"
    );

    let generated_key = &commit_result.generated_keys[0];
    assert!(
        generated_key.starts_with(b"user_score_"),
        "Generated key should start with prefix"
    );
    assert_eq!(
        generated_key.len(),
        21,
        "Generated key should be 21 bytes (11 prefix + 10 versionstamp)"
    );
    ...
    assert_eq!(
        value,
        Some(b"100".to_vec()),
        "Should be able to read back the versionstamped key"
    );
...
async fn test_versionstamped_value_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tx.set_versionstamped_value(
        b"user_session",
        b"session_\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
        None,
    )?;
    ...
    assert_eq!(
        commit_result.generated_keys.len(),
        0,
        "Should have no generated keys for value operation"
    );
    assert_eq!(
        commit_result.generated_values.len(),
        1,
        "Should have one generated value"
    );

    let generated_value = &commit_result.generated_values[0];
    assert!(
        generated_value.starts_with(b"session_"),
        "Generated value should start with prefix"
    );
    assert_eq!(
        generated_value.len(),
        18,
        "Generated value should be 18 bytes (8 prefix + 10 versionstamp)"
    );
    ...
    assert_eq!(
        ...
        "Should be able to read back the versionstamped value"
    );
...
async fn test_mixed_versionstamped_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tx.set_versionstamped_key(
        b"log_entry_\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
        b"event_data",
        None,
    )?;
    tx.set_versionstamped_value(
        b"event_value",
        b"data_\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
        None,
    )?;
    ...
    // Verify formats
    assert!(
        generated_key.starts_with(b"log_entry_"),
        "Generated key should start with prefix"
    );
    assert!(
        generated_value.starts_with(b"data_"),
        "Generated value should start with prefix"
    );
...
async fn test_versionstamp_uniqueness() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tx1.set_versionstamped_key(
        b"test_\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
        b"value1",
        None,
    )?;
...
async fn test_backward_compatibility_commit() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tx.set_versionstamped_key(
        b"compat_test_\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
        b"data",
        None,
    )?;
...
async fn test_versionstamp_buffer_size_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        result
            .unwrap_err()
            .to_string()
            .contains("at least 10 bytes"),
        "Error should mention 10 byte requirement"
    );
    ...
    assert_eq!(
        result.generated_values.len(),
        1,
        "Should generate one value"
    );
