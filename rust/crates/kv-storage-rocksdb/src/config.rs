...
impl Config {
    ...
    pub fn create_database_factory(&self) -> crate::DatabaseFactory {
        match self.deployment.mode {
            DeploymentMode::Standalone => crate::DatabaseFactory::standalone(self.clone()),
            DeploymentMode::Replicated => {
                let instance_id = self.deployment.instance_id.unwrap_or(0);
                crate::DatabaseFactory::replicated(self.clone(), instance_id)
            }
        }
    }
}
